...
registry.register(
	...
	async ({
		...
		threadId: string;
		data: string;
		filename: Parameters<(typeof messengerClient)['sendVideo']>[2];
		options?: Parameters<(typeof messengerClient)['sendVideo']>[3];
	}) => {
		const result = await messengerClient.sendVideo(
			BigInt(threadId),
